...
namespace EduEngine
{
	...
	{
	public:
		GBuffer(int count, const DXGI_FORMAT* formats, DXGI_FORMAT accumBuffFormat);
		...
		ID3D12Resource* GetAccumBuffer() const;
		D3D12_CPU_DESCRIPTOR_HANDLE GetAccumBuffRTVView() const;
		D3D12_GPU_DESCRIPTOR_HANDLE GetAccumBuffSRVView() const;
	...
	private:
		...
		std::unique_ptr<TextureD3D12> m_AccumBuffer;
	};
}